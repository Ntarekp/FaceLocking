...
def _estimate_norm_5pt(
    ...
    kps order: [Leye, Reye, Nose, Lmouth, Rmouth]
    """
...
class Haar5ptDetector:
    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 1) -> List[FaceKpsBox]:
        ...
        if kps is None:
            if self.debug:
                print("[haar_5pt] Haar hit but FaceMesh none -> reject")
            return []
        ...
        if inside.mean() < 0.60:
            if self.debug:
                print("[haar_5pt] FaceMesh not consistent with Haar box -> reject")
            return []

        if not _kps_span_ok(kps, max(10.0, 0.18 * w)):
            if self.debug:
                print("[haar_5pt] 5pt geometry failed -> reject")
            return []
