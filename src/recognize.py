...
class FaceDet: 
   ...
   action_kps: Optional[np.ndarray] = None # (12,2) for action detection
...
def select_identity(names):
    ...
    while True:
        try:
            val = input(f"Select identity to lock (1-{len(names)}): ")
            idx = int(val) - 1
            if 0 <= idx < len(names):
                return names[idx]
...
class HaarFaceMesh5pt: 
   ...
   def _roi_facemesh_5pt(self, roi_bgr: np.ndarray) -> Optional[Tuple[np.ndarray, np.ndarray]]: 
       H, W = roi_bgr.shape[:2] 
       if H < 20 or W < 20: 
           return None 
...
self.IDX_MOUTH_RIGHT] 
       ...
       # Extract action landmarks
       action_pts = []
       for i in self.IDXS_ACTIONS:
           p = lm[i]
           action_pts.append([p.x * W, p.y * H])
       action_kps = np.array(action_pts, dtype=np.float32)

       return kps, action_kps 
   ...
   def detect(self, frame_bgr: np.ndarray, max_faces: int = 5) -> List[FaceDet]: 
       ...
       for (x, y, w, h) in faces: 
           ...
           res = self._roi_facemesh_5pt(roi) 
           if res is None: 
               if self.debug: 
                   print("[recognize] FaceMesh none for ROI -> skip") 
               continue 
           kps_roi, action_kps_roi = res
           ...
           action_kps = action_kps_roi.copy()
           action_kps[:, 0] += float(rx1)
           action_kps[:, 1] += float(ry1)
           ...
           out.append( 
               FaceDet( 
                   ...
                   action_kps=action_kps.astype(np.float32),
               ) 
           ) 
...
def main(): 
   ...
   prev_landmarks = None
   missing_frames = 0
   max_missing_frames = 60  # ~2 seconds at 30fps
   ...
   while True:
       ...
       if locked:
           ...
           for i, f in enumerate(faces):
               curr_bbox = (f.x1, f.y1, f.x2, f.y2)
               if face_lock.locked_bbox is not None:
                   iou = compute_iou(face_lock.locked_bbox, curr_bbox)
                   if iou > best_iou:
                       best_iou = iou
                       best_idx = i
           if best_iou < 0.3:
       ...
       for i, f in enumerate(faces):
           ...
           if locked and i == best_idx:
               is_target = True
               found_locked = True
               
               # Detect actions using action landmarks (12 pts)
               actions = face_lock.detect_actions(prev_bbox, prev_landmarks, (f.x1, f.y1, f.x2, f.y2), f.action_kps)
               prev_bbox = (f.x1, f.y1, f.x2, f.y2)
               prev_landmarks = f.action_kps
               
               # Update lock
               face_lock.update((f.x1, f.y1, f.x2, f.y2), f.kps, mr.name, mr.similarity)
               
           elif not locked:
               # Try to lock
               if face_lock.try_lock(mr.name, (f.x1, f.y1, f.x2, f.y2), f.kps, mr.similarity, threshold=LOCK_CONFIDENCE_THRESHOLD):
                   locked = True
                   found_locked = True
                   is_target = True
                   prev_bbox = (f.x1, f.y1, f.x2, f.y2)
           ...
           if is_target:
               ...
               cv2.rectangle(vis, (f.x1, f.y1), (f.x2, f.y2), (255, 0, 0), 3)
               cv2.putText(vis, f"LOCKED: {locked_identity}", (f.x1, max(0, f.y1 - 40)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
               
               # Show actions
               y_off = 0
               for act_name, act_desc in list(face_lock.action_buffer)[::-1]: # Show newest on top
                    cv2.putText(vis, act_desc, (10, 60 + y_off), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                    y_off += 25
           else:
       ...
       # If locked but not found, increment missing frames
       if locked and not found_locked:
           missing_frames += 1
           if missing_frames > max_missing_frames:
               face_lock.release_lock()
               prev_bbox = None
               prev_landmarks = None
               missing_frames = 0
       else:
       ...
       if fps is not None:
           header += f"  fps={fps:.1f}"
       if face_lock.is_locked():
           header += f"  LOCKED: {locked_identity}"
